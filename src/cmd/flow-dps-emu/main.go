...
import (
	...
	"github.com/onflow/api-service/m/v2/cmd/flow-dps-emu/builder"
	"github.com/onflow/api-service/m/v2/cmd/proxy"
	srv "github.com/onflow/api-service/m/v2/cmd/service"
)

func main() {
	serviceBuilder := builder.NewFlowDPSEmulatorBuilder()
	...
	serviceBuilder.
		...
		Module("RPC Engine", func(node *srv.ServiceConfig) error {
			rpcEng, err := engine.NewDPS(node.Logger, serviceBuilder.RpcConf, serviceBuilder.Api)
			if err != nil {
				serviceBuilder.ServiceConfig.Logger.Info().Err(err)
				return err
			}
